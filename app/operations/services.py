...
def ensure_workflow(folio):
    ...
    workflow = FolioWorkflow(folio_id=folio.id, step_1_ui_folio=True)
    return workflow
...
def ensure_deliverables(folio):
    existing_codes = {
        item.code
        for item in FolioDeliverableItem.query.filter_by(folio_id=folio.id).all()
    }
...
def update_folio_operational_status(
    *,
    folio,
    workflow,
    deliverables,
